...
pipeline {
    ...
    environment {
        ...
        harborRepo = "maxblog-admin-fe"
        Container_port = "9602" // 启用随机端口，会被赋值
        Host_port = "9602" // 启用随机端口，会被赋值
        JobName = "maxblog-admin-fe/main"
        ...
        stage("Checkout") {
            steps {
                ...
                timeout(time: 3, unit: "MINUTES"){
                    checkout([$class: "GitSCM", branches: [[name: '**']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/liuzhaomax/maxblog-admin-fe.git']]])
                }
                echo "--------------------- Checkout End ---------------------"
            }
        }
